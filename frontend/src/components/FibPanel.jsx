const RET_LEVELS = ['0.786', '0.618', '0.500', '0.382', '0.236']
const EXT_LEVELS = ['0.236', '0.382', '0.500', '0.618', '0.786']
...
function isNear(price, level, range) {
  ...
  return Math.abs(Number(price) - Number(level)) <= range * 0.005
}
    ...
    return (
      <div className="bg-gray-900 rounded-lg p-4 min-h-72">
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Fibonacci (D1)</h2>
  ...
  return (
    <div className="bg-gray-900 rounded-lg p-4 min-h-72">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs mb-3">
        <span className="text-gray-500">Swing High <span className="text-gray-300 font-mono">{fmt(phigh)}</span></span>
        <span className="text-gray-500">Swing Low <span className="text-gray-300 font-mono">{fmt(plow)}</span></span>
        <span className="text-gray-500">Direction <span className={`font-mono ${data.direction === 'bullish' ? 'text-green-400' : 'text-red-400'}`}>{data.direction === 'bullish' ? 'Bullish' : 'Bearish'}</span></span>
        <span className="text-gray-500">Range <span className="text-gray-300 font-mono">{fmt(range)} pts</span></span>
      </div>

      <div className="space-y-0.5 font-mono text-xs">
        <p className="text-gray-600 text-[10px] uppercase tracking-wider px-2">▲ Retracement</p>
        {RET_LEVELS.map(key => (
          <LevelRow key={`r-${key}`} label={key} value={data.levels?.[key]}
            currentPrice={price} range={range} colorClass="text-green-400" />
        ))}

        <div className={`grid grid-cols-[4.5rem_1fr] items-center border-y border-gray-700 px-2 py-1 my-1 ${isNear(price, plow, range) ? 'bg-yellow-500/30 text-yellow-100 animate-pulse' : 'text-gray-300'}`}>
          <span className="text-gray-500">0.000</span>
          <span className="text-right font-mono">{fmt(plow)} <span className="text-[10px] text-gray-500">(Swing Low)</span></span>
        </div>

        <p className="text-gray-600 text-[10px] uppercase tracking-wider px-2">▼ Extension</p>
        {EXT_LEVELS.map(key => (
